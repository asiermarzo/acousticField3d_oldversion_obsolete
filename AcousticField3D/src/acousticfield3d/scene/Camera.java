...
public class Camera extends Entity{
    ...
    public void activateObservation(boolean activate, final Vector3f point){
        if (activate){
            ...
            inclination = FastMath.acos( diff.y / distance);
            azimuth = FastMath.atan2(-diff.z, diff.x);
    ...
    public void updateObservation(){
        Vector3f translation = getTransform().getTranslation();
        translation.x = observationPoint.x + distance * FastMath.sin( inclination ) * FastMath.cos( azimuth );
        translation.z = observationPoint.z - distance * FastMath.sin( inclination ) * FastMath.sin( azimuth );
        translation.y = observationPoint.y + distance * FastMath.cos( inclination );
    ...
    public void moveAzimuthAndInclination(float diffAzimuth, float diffInclination) {
        ...
        if (inclination < margin) {
            inclination = margin;
        } else if (inclination > FastMath.PI - margin) {
            inclination = FastMath.PI - margin;
        }
    }
