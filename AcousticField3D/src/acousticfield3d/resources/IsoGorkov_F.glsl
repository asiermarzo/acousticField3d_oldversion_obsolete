...
vec3 getNormalAt(vec3 p, float h){
    vec3 n = vec3( gorkovAt(p - vec3(h,0.0,0.0) ) - gorkovAt(p + vec3(h,0.0,0.0)),
                   gorkovAt(p - vec3(0.0,h,0.0) ) - gorkovAt(p + vec3(0.0,h,0.0)),
                   gorkovAt(p - vec3(0.0,0.0,h) ) - gorkovAt(p + vec3(0.0,0.0,h)) );
    return normalize(n);
}
