...
float phaseAt(vec3 p){
    vec2 field = fieldAt(p);
    return atan(field.y, field.x);
}
vec3 getNormalAt(vec3 p, float h){
    
    vec3 n = vec3( phaseAt(p - vec3(h,0.0,0.0) ) - phaseAt(p + vec3(h,0.0,0.0)),
                   phaseAt(p - vec3(0.0,h,0.0) ) - phaseAt(p + vec3(0.0,h,0.0)),
                   phaseAt(p - vec3(0.0,0.0,h) ) - phaseAt(p + vec3(0.0,0.0,h)) );
    return normalize(n);
}
