...
//</editor-fold>
    ...
    public void calcGorkovGradient(Vector3f position, Vector3f forces, Renderer r, double[] phases){
        setPosition(position);
        initFieldConstants(r);
    ...
    public void allocateAndInit(Renderer r){
        allocate(r);
        initFieldConstants(r);
    }
    ...
    public static float calcDir(float tx, float ty, float tz,final Vector3f n, float k){
        ...
        final float angle = FastMath.acos( dot / n.length() / FastMath.sqrt(d2) );
        final float sinAngle = FastMath.sin( angle );
        ...
        return (float)FastMath.j1(dum) / dum;
    ...
    public void initFieldConstantsOnlyForAmp(Renderer r){
        ...
        for(int i = 0; i < nTotalTrans; ++i){
            ...
            float kd = k*d;
            float cosKD = FastMath.cos(kd);
            float sinKD = FastMath.sin(kd);
    ...
    public void initFieldConstants(Renderer r){
        reset();
        ...
        for(int i = 0; i < nTotalTrans; ++i){
            ...
            final float dot = x*nor.x + y*nor.y + z*nor.z;
            final float d2 = x*x + y*y + z*z;
            final float angle = FastMath.acos( dot / nor.length() / FastMath.sqrt(d2) );
            final float sinAngle = FastMath.sin( angle );
            //final float sinAngle = FastMath.sqrt(1.0f - (dot*dot / d2) );
            System.out.println( "angle " + sinAngle);
        }*/
        ...
        M1 = r.getVpVol() * r.getVkPre();
        M2 = r.getVpVol() * r.getVkVel()*r.getVkPreToVel()*r.getVkPreToVel();   
