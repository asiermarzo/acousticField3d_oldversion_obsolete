...
public class Animation {
    ...
    public void switchTopBottom(Simulation simulation) {
        final ArrayList<Transducer> trans = simulation.getTransducers();
        final int size = trans.size();
        final int halfSize = size / 2;
        for(int i = 0; i < halfSize; ++i){
            final Transducer a = trans.get(i);
            final Transducer b = trans.get(i + halfSize);
            for(AnimKeyFrame key : keyFrames.getElements()){
                TransState at = key.getTransStates().get(a);
                TransState bt = key.getTransStates().get(b);
                if (at != null && bt != null){
                    key.getTransStates().remove(a);
                    key.getTransStates().remove(b);
                    at.setTransducer(b);
                    bt.setTransducer(a);
                    key.getTransStates().put(b, at);
                    key.getTransStates().put(a, bt);
                }
            }
        }
    }
